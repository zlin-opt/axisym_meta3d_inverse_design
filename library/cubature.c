...
static PetscReal errMax(unsigned fdim, const esterr *ee)
{
     PetscReal errmax = 0;
     unsigned k;
     for (k = 0; k < fdim; ++k)
...
typedef struct {
     unsigned dim;
     PetscReal *data;	/* length 2*dim = center followed by half-widths */
     PetscReal vol;	/* cache volume = product of widths */
} hypercube;

static PetscReal compute_vol(const hypercube *h)
{
     unsigned i;
     PetscReal vol = 1;
     for (i = 0; i < h->dim; ++i)
...
static hypercube make_hypercube(unsigned dim, const PetscReal *center, const PetscReal *halfwidth)
{
     unsigned i;
     hypercube h;
     h.dim = dim;
     h.data = (PetscReal *) malloc(sizeof(PetscReal) * dim * 2);
     h.vol = 0;
     if (h.data) {
	  for (i = 0; i < dim; ++i) {
	       h.data[i] = center[i];
...
static hypercube make_hypercube_range(unsigned dim, const PetscReal *xmin, const PetscReal *xmax)
{
...
static PetscInt cut_region(region *R, region *R2)
{
...
typedef PetscInt (*evalError_func)(struct rule_s *r,
...
typedef struct rule_s {
     ...
     PetscReal *pts; /* points to eval: num_regions * num_points * dim */
     PetscReal *vals; /* num_regions * num_points * fdim */
...
static PetscInt alloc_rule_pts(rule *r, unsigned num_regions)
{
     if (num_regions > r->num_regions) {
	  ...
	  r->pts = (PetscReal *) malloc(sizeof(PetscReal) * 
				     (num_regions
...
static PetscInt eval_regions(unsigned nR, region *R, 
 ...
 */
static void evalR_Rfs(PetscReal *pts, unsigned dim, PetscReal *p, const PetscReal *c, const PetscReal *r)
{
     unsigned i;
     ...
     for (i = 0;; ++i) {
	  ...
	  memcpy(pts, p, sizeof(PetscReal) * dim); pts += dim;
...
static void evalRR0_0fs(PetscReal *pts, unsigned dim, PetscReal *p, const PetscReal *c, const PetscReal *r)
{
     unsigned i, j;
...
static void evalR0_0fs4d(PetscReal *pts, unsigned dim, PetscReal *p, const PetscReal *c,
			 const PetscReal *r1, const PetscReal *r2)
{
     unsigned i;
...
typedef struct {
     ...
     PetscReal *widthLambda, *widthLambda2, *p;

     /* dimension-dependent constants */
     PetscReal weight1, weight3, weight5;
     PetscReal weightE1, weightE3;
} rule75genzmalik;

#define real(x) ((PetscReal)(x))
#define to_int(n) ((int)(n))

static PetscInt isqr(int x)
{
     return x * x;
}
...
static PetscInt rule75genzmalik_evalError(rule *r_, unsigned fdim, integrand_v f, void *fdata, unsigned nR, region *R)
{
     /* lambda2 = sqrt(9/70), lambda4 = sqrt(9/10), lambda5 = sqrt(9/19) */
     const PetscReal lambda2 = 0.3585685828003180919906451539079374954541;
     const PetscReal lambda4 = 0.9486832980505137995996680633298155601160;
     const PetscReal lambda5 = 0.6882472016116852977216287342936235251269;
     const PetscReal weight2 = 980. / 6561.;
     const PetscReal weight4 = 200. / 19683.;
     const PetscReal weightE2 = 245. / 486.;
     const PetscReal weightE4 = 25. / 729.;
     const PetscReal ratio = (lambda2 * lambda2) / (lambda4 * lambda4);

     rule75genzmalik *r = (rule75genzmalik *) r_;
     unsigned i, j, iR, dim = r_->dim;
     size_t npts = 0;
     PetscReal *diff, *pts, *vals;

     if (alloc_rule_pts(r_, nR)) return FAILURE;
     pts = r_->pts; vals = r_->vals;

     for (iR = 0; iR < nR; ++iR) {
	  const PetscReal *center = R[iR].h.data;
	  const PetscReal *halfwidth = R[iR].h.data + dim;
	...
	for each hypercube */
     ...
     for (j = 0; j < fdim; ++j) {
	  const PetscReal *v = vals + j;
#         define VALS(i) v[fdim*(i)]	       
	  for (iR = 0; iR < nR; ++iR) {
	       PetscReal result, res5th;
	       PetscReal val0, sum2=0, sum3=0, sum4=0, sum5=0;
	       unsigned k, k0 = 0;
	       ...
	       for (k = 0; k < dim; ++k) {
		    PetscReal v0 = VALS(k0 + 4*k);
		    PetscReal v1 = VALS((k0 + 4*k) + 1);
		    PetscReal v2 = VALS((k0 + 4*k) + 2);
		    PetscReal v3 = VALS((k0 + 4*k) + 3);
...
static rule *make_rule75genzmalik(unsigned dim, unsigned fdim)
{
	...
	slow and is superseded by other methods (e.g. Monte-Carlo). */
     ...
     r->p = (PetscReal *) malloc(sizeof(PetscReal) * dim * 3);
...
static PetscInt rule15gauss_evalError(rule *r,
				      unsigned fdim, integrand_v f, void *fdata,
				      unsigned nR, region *R)
{
	...
	L. W. Fullerton, Bell Labs, Nov. 1981. */
     const unsigned n = 8;
     const PetscReal xgk[8] = {  /* abscissae of the 15-point kronrod rule */
     ...
     static const PetscReal wg[4] = {  /* weights of the 7-point gauss rule */
	  0.129484966168869693270611432679082,
	  0.279705391489276667901467771423780,
	  0.381830050505118944950369775488975,
	  0.417959183673469387755102040816327
     };
     static const PetscReal wgk[8] = { /* weights of the 15-point kronrod rule */
     ...
     for (iR = 0; iR < nR; ++iR) {
	  const PetscReal center = R[iR].h.data[0];
	  const PetscReal halfwidth = R[iR].h.data[1];

	  pts[npts++] = center;

	  for (j = 0; j < (n - 1) / 2; ++j) {
	       PetscInt j2 = 2*j + 1;
	       PetscReal w = halfwidth * xgk[j2];
	       pts[npts++] = center - w;
	       pts[npts++] = center + w;
	  }
	  for (j = 0; j < n/2; ++j) {
     ...
     for (k = 0; k < fdim; ++k) {
          const PetscReal *vk = vals + k;
	  for (iR = 0; iR < nR; ++iR) {
	       const PetscReal halfwidth = R[iR].h.data[1];
	       PetscReal result_gauss = vk[0] * wg[n/2 - 1];
	       PetscReal result_kronrod = vk[0] * wgk[n - 1];
	       PetscReal result_abs = fabs(result_kronrod);
	       PetscReal result_asc, mean, err;

	       /* accumulate integrals */
	       npts = 1;
	       for (j = 0; j < (n - 1) / 2; ++j) {
		    PetscInt j2 = 2*j + 1;
		    PetscReal v = vk[fdim*npts] + vk[fdim*npts+fdim];
		    result_gauss += wg[j] * v;
		  ...
		  ... not completely clear to me why we don't just use
	       ...
	       if (result_asc != 0 && err != 0) {
		    PetscReal scale = pow((200 * err / result_asc), 1.5);
		    err = (scale < 1) ? result_asc * scale : result_asc;
	       }
	       if (result_abs > DBL_MIN / (50 * DBL_EPSILON)) {
		    PetscReal min_err = 50 * DBL_EPSILON * result_abs;
...
static PetscInt heap_push(heap *h, heap_item hi)
{
     PetscInt insert;
...
static PetscInt heap_push_many(heap *h, size_t ni, heap_item *hi)
{
     size_t i;
     for (i = 0; i < ni; ++i)
...
static PetscInt converged(unsigned fdim, const esterr *ee,
			  PetscReal reqAbsError, PetscReal reqRelError, error_norm norm)
{
  unsigned j;
#    define SQR(x) ((x) * (x))
  switch (norm) {
  ...
  case ERROR_PAIRED:
    for (j = 0; j+1 < fdim; j += 2) {
      PetscReal maxerr, serr, err, maxval, sval, val;
  ...
  case ERROR_L2: {
    PetscReal maxerr = 0, maxval = 0, serr, sval, err = 0, val = 0;
...
static PetscInt rulecubature(rule *r, unsigned fdim, 
			     integrand_v f, void *fdata, 
			     const hypercube *h, 
			     size_t maxEval,
			     PetscReal reqAbsError, PetscReal reqRelError,
			     error_norm norm,
			     PetscReal *val, PetscReal *err, PetscInt parallel)
{
     size_t numEval = 0;
     heap regions;
     unsigned i, j;
...
static PetscInt cubature(unsigned fdim, integrand_v f, void *fdata, 
			 unsigned dim, const PetscReal *xmin, const PetscReal *xmax, 
			 size_t maxEval, PetscReal reqAbsError, PetscReal reqRelError, 
			 error_norm norm,
			 PetscReal *val, PetscReal *err, PetscInt parallel)
{
     rule *r;
...
static PetscInt fv(unsigned ndim, size_t npt,
		   const PetscReal *x, void *d_,
...
PetscInt hcubature(unsigned fdim, integrand f, void *fdata, 
		   unsigned dim, const PetscReal *xmin, const PetscReal *xmax, 
		   size_t maxEval, PetscReal reqAbsError, PetscReal reqRelError, 
		   error_norm norm,
		   PetscReal *val, PetscReal *err)
...
PetscReal quadrature(integrand f, PetscReal *params,
		     const PetscReal xmin, const PetscReal xmax,
		     size_t maxEval, PetscReal abserr, PetscReal relerr,
		     PetscReal *err)
{


  PetscReal x0=xmin, x1=xmax;
  PetscReal result;
  hcubature(1, f, params,
	    1, &x0, &x1,
